...
export async function deployBackendForClient(clientName: string) {
    ...
    try {
        ...
        const appJsContent = `// Auto-generated by Systego Provisioner
...
try {
    ...
    console.log('[Systego Boot] Files in app root:', files.join(', '));
    
    if (fs.existsSync(path.join(__dirname, 'dist'))) {
        const distFiles = fs.readdirSync(path.join(__dirname, 'dist'));
        console.log('[Systego Boot] Files in dist/:', distFiles.join(', '));
        
        if (fs.existsSync(path.join(__dirname, 'dist', 'src'))) {
            const srcFiles = fs.readdirSync(path.join(__dirname, 'dist', 'src'));
            console.log('[Systego Boot] Files in dist/src/:', srcFiles.join(', '));
        } else {
            console.error('[Systego Boot] ERROR: dist/src/ directory does NOT exist!');
        }
    } else {
        console.error('[Systego Boot] ERROR: dist/ directory does NOT exist!');
    }
} catch (e) {
    console.error('[Systego Boot] Directory scan error:', e.message);
}
...
try {
    ...
    const errorMsg = '[Systego Boot] FATAL: ' + err.message + '\\n' + err.stack;
    console.error(errorMsg);
    fs.writeFileSync(path.join(__dirname, 'startup-error.log'), errorMsg);
    
    // Start a minimal HTTP server that shows the error
